...
- 系統主要使用分離的 YAML repository 作為資料來源：`settings.yaml`（系統設定）、`activities.yaml`（活動與模板）、`activity-results.yaml`（結果與 Donate 紀錄），另以 `obs-assets.yaml` 保存 OBS 資產 metadata
