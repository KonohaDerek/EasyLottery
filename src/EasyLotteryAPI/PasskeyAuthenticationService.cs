...
public sealed class PasskeyAuthenticationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new PasskeyBeginResult(flow, ParseOptions(optionsJson));
    }
    ...
    public async Task<PasskeyBeginResult> BeginAddAsync(string? name, CancellationToken cancellationToken)
    {
        ...
        var optionsJson = CreateOptionsJson(_options.Email, AdminPasskeyOptions.AddFlow, state.Credentials);
        await _store.SetPendingAsync(
            AdminPasskeyOptions.AddFlow,
            new PasskeyPendingCeremony
            {
        ...
        return new PasskeyBeginResult(AdminPasskeyOptions.AddFlow, ParseOptions(optionsJson));
    }
    ...
    public async Task<PasskeyDeviceResponse> FinishAddAsync(
        ...
        CancellationToken cancellationToken)
    {
        var pending = await _store.TakePendingAsync(AdminPasskeyOptions.AddFlow, cancellationToken);
        ...
        IReadOnlyCollection<PasskeyCredentialRecord> credentials)
    {
        ...
        if (flow == AdminPasskeyOptions.LoginFlow)
        {
    ...
    private static string NormalizeFlow(string? flow)
    {
        ...
        if (normalized is not (AdminPasskeyOptions.RegisterFlow or AdminPasskeyOptions.LoginFlow))
