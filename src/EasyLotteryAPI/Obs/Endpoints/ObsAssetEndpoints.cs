...
{
    private static readonly IReadOnlyList<(string Prefix, ObsAssetKind Kind)> ContentTypeKinds =
    [
        ("image/", ObsAssetKind.Image),
        ("audio/", ObsAssetKind.Audio),
    ...
    private static ObsAssetKind InferKind(string? contentType)
    {
        ...
        foreach (var (prefix, kind) in ContentTypeKinds)
        {
            if (normalized.StartsWith(prefix, StringComparison.Ordinal)) return kind;
        }
